...
- When importing a Chamber package that is a concrete adapter around a third-party implementation, make the alias say it is the Chamber adapter, not the upstream project.
  Example: use `chamberDirectoryProvisioner` for `github.com/donglin-wang/chamber/pkg/bundle/internal/directory` rather than `umoci` inside factory packages.
- Daemon and CI composition should usually import root SDK contract packages
  such as `chamberImage`, `chamberBundle`, and `chamberRuntime`, plus
  `chamberImageFactory`, `chamberBundleFactory`, or `chamberRuntimeFactory`
...
- Root SDK packages such as `pkg/image`, `pkg/bundle`, and `pkg/runtime` own public contracts, schemas, config structs, fixed string constants, and package-level helper functions.
- Factory packages such as `pkg/image/factory`, `pkg/bundle/factory`, and `pkg/runtime/factory` own object-specific constructors such as `NewStore`, `NewProvisioner`, and `NewRuntime`. Factory constructors perform shared validation, private-directory setup, implementation name/capability checks when the package has multiple implementations, and dispatch to concrete implementations.
- Concrete implementation packages live under `pkg/<package>/internal/<implementation>`, import only their package's root contract/config package, and expose an ordinary `New` constructor for the factory package to call. Concrete constructors should do implementation-specific initialization only; shared validation and directory creation belong in the factory constructor.
- If a package grows multiple implementations, add the implementation name/capability vocabulary to that package's root config/contracts and keep the implementation list in the factory package. Do not use registration, `init`, side-effect imports, or static-analysis enforcement unless Chamber intentionally grows a plugin model.

Important current boundaries:

- `pkg/image`: public image-operation contracts, pull/build/list/remove request types, image-root config, Buildah config, image metadata records, and small layout helpers. Use `image/factory.NewStore`; `image.Store` owns one shared OCI image layout below the configured root plus store-local metadata and temporary operation directories. `PullIfMissing` reuses valid metadata/layout records, while `PullAlways` refreshes the shared-layout reference and preserves `CreatedAt`. SDK callers own root placement, process-level coordination, cleanup policy, and recovery.
- `pkg/image/internal/store`, `pkg/image/internal/metadata`, `pkg/image/internal/registry`, and `pkg/image/internal/buildah`: concrete image-store orchestration, JSON metadata, go-containerregistry pull mechanics, and Buildah-worker-backed Dockerfile build mechanics. Registry/build helpers write temporary OCI layouts; the store validates, copies reachable blobs into the shared layout, rewrites the shared index, and writes metadata last. `Store.Build` uses a configured local `buildah-worker` executable or downloads the managed worker, and keeps Buildah graph, run, and temp roots under the configured image root.
- `pkg/bundle`: public bundle provisioning contract, bundle-root config, `ProcessSpec`, and provisioner-applied `Mounts`. Use `bundle/factory.NewProvisioner`; `pkg/bundle.Config.Name` selects the concrete provisioner implementation, and the factory constructor owns implementation name/capability checks before dispatch. `ProcessSpec.Terminal` is a pointer so callers can distinguish "leave the image default alone" from "force true/false". Do not expose future runtime-applied rootfs mount hooks before a runtime implementation supports them.
