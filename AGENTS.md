...
- When importing a Chamber package that is a concrete adapter around a third-party implementation, make the alias say it is the Chamber adapter, not the upstream project.
  Example: use `chamberDirectoryProvisioner` for `github.com/donglin-wang/chamber/pkg/bundle/internal/directory` rather than `umoci` inside root constructor packages.
- Daemon and CI composition should usually import root SDK constructor packages
  such as `chamberImage`, `chamberBundle`, and `chamberRuntime`, plus their
  `shared` packages for config/contracts. Concrete adapter aliases such as
  `chamberImagePuller` or `chamberDirectoryProvisioner` belong inside root
  constructor packages or implementation-specific tests, not normal daemon
  composition.
...
Canonical SDK package shape:

- Root SDK packages such as `pkg/image`, `pkg/bundle`, and `pkg/runtime` own object-specific shared constructors such as `NewPuller`, `NewProvisioner`, and `NewRuntime`. Root constructors perform shared validation, private-directory setup, implementation name/capability checks when the package has multiple implementations, and dispatch to concrete implementations.
- Each SDK package keeps contracts, schemas, config structs, fixed string constants, and package-level helper functions under `pkg/<package>/shared`. Callers and implementation packages import these shared types directly. Do not add root-package type aliases, constant aliases, or wrapper functions just to preserve old imports; update call sites to import `shared` instead.
- Concrete implementation packages live under `pkg/<package>/internal/<implementation>`, import only their package's `shared` package for contracts/config, and expose an ordinary `New` constructor for the root constructor to call. Concrete constructors should do implementation-specific initialization only; shared validation and directory creation belong in the root constructor.
- If a package grows multiple implementations, add the implementation name/capability vocabulary to that package's `shared` config/contracts and keep the implementation list in the root constructor package. Do not use registration, `init`, side-effect imports, or static-analysis enforcement unless Chamber intentionally grows a plugin model.

Important current boundaries:

- `pkg/image`: public constructor facade for image operations. Use object-specific root constructors such as `image.NewPuller`; future image operations should follow the same shape, for example `NewPusher` or `NewInspector`. `pkg/image/shared` owns the public puller contract, pull request platform/auth/policy fields, image-root config, and small layout helpers. Configured image roots are the source of truth for pulled-image storage; concrete pullers derive destinations directly with `DestinationForCanonicalImage(root, ref, platform)`. `PullIfMissing` reuses local layouts, while `PullAlways` refreshes mutable references. SDK callers own root placement, locking, cleanup, and recovery.
- `pkg/image/internal/puller`: concrete OCI image puller using `go-containerregistry`. It honors platform/auth request fields and must keep the atomic temp-then-rename layout write behavior. Future sibling implementation packages may include `pkg/image/internal/pusher` and `pkg/image/internal/inspector`.
- `pkg/bundle`: public constructor facade for bundle provisioners. Use `bundle.NewProvisioner`; `pkg/bundle/shared.Config.Name` selects the concrete provisioner implementation, and the root constructor owns implementation name/capability checks before dispatch. `pkg/bundle/shared` owns the public bundle provisioning contract, bundle-root config, `ProcessSpec`, and provisioner-applied `Mounts`. `ProcessSpec.Terminal` is a pointer so callers can distinguish "leave the image default alone" from "force true/false". Do not expose future runtime-applied rootfs mount hooks before a runtime implementation supports them.
...
- `pkg/shared/errors`: canonical public Chamber error-code taxonomy. The daemon and SDK adapters should use these durable codes for contract errors and response mapping.
  ...
  validate an image layout before opening it; if validation fails, return
  `ErrInvalidImageLayout`, if a later unpack step fails, return
  `ErrBundlePrepareFailed`. Avoid `classify...` helpers and `wrap...Error`
  helpers that exist only to preserve or reformat nested error codes. Add a
  precise Chamber validation code when `ErrInvalidRequest` would hide a stable
