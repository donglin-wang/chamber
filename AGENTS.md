...
- Daemon and CI composition should usually import root SDK contract packages
  ...
  as `chamberImagePuller` or `chamberDirectoryProvisioner` belong inside
  factory packages or implementation-specific tests, not normal daemon
  composition.
...
- Factory packages such as `pkg/image/factory`, `pkg/bundle/factory`, and `pkg/runtime/factory` own object-specific constructors such as `NewPuller`, `NewProvisioner`, and `NewRuntime`. Factory constructors perform shared validation, private-directory setup, implementation name/capability checks when the package has multiple implementations, and dispatch to concrete implementations.
...
- `pkg/image`: public image-operation contracts, pull request platform/auth/policy fields, image-root config, and small layout helpers. Use object-specific factory constructors such as `image/factory.NewPuller`; future image operations should follow the same shape, for example `NewPusher` or `NewInspector` in the factory package. Configured image roots are the source of truth for pulled-image storage; concrete pullers derive destinations directly with `DestinationForCanonicalImage(root, ref, platform)`. `PullIfMissing` reuses local layouts, while `PullAlways` refreshes mutable references. SDK callers own root placement, locking, cleanup, and recovery.
