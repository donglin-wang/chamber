...
### Phase 1: Focused Research and Design

- Choose implementation language.
- Select initial libraries for image pull, storage, OCI spec generation, runtime execution, and minimal networking if needed.
- Define the API schema.
- Define the persistent state schema.
- Define the boundary between Chamber's node API and the future distributed coordinator.
- Define per-user default paths, socket permissions, lock placement, and unsafe-path rejection rules.
- Write concurrency invariants.
- Write per-user isolation invariants.
- Build small spikes for rootless run, pull, and cleanup.

### Phase 2: Minimal Daemon

- Implement daemon startup.
- Implement Unix socket API.
- Implement per-user path resolution and ownership/permission validation.
- Implement state store.
- Implement operation records.
- Implement event log.
- Implement basic locking and leases.

### Phase 3: Image and Runtime MVP

- Implement image pull.
- Implement local image metadata.
- Implement container create/start/run.
- Integrate `crun` or `runc`.
- Implement logs and exit status.
- Implement cancellation/stop semantics.
- Implement container remove.

### Phase 4: GC and Recovery

- Implement mark-delete for images.
- Implement lease-aware GC.
- Implement startup reconciliation.
- Add crash tests around partial operations.

### Phase 5: MVP Hardening

- Add stress tests for concurrent pull/list/run/remove/cancel/GC.
- Add multi-user coexistence tests.
- Add diagnostics for paths, daemon identity, active operations, leases, and cleanup blockers.
- Add minimal API client tooling.
- Document operational limits and unsupported features.

### Phase 6: Post-MVP Evaluation Gate

- Evaluate whether Chamber has a real consumer workload.
- Measure whether the daemon contract is better than shelling out to existing tools for that workload.
- Decide which deferred features, if any, are justified by observed use.
- Keep the default answer to deferred features as "no" until a concrete workload requires them.
